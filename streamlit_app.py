...
# Calendar & Task Manager constants
TM_STATUS_OPTIONS   = ["To-Do", "In Progress", "Paused", "In Review", "Done"]
TM_PRIORITY_OPTIONS = ["Urgent", "High", "Medium", "Low"]
MEETING_STATUS_OPTIONS = ["Scheduled", "Confirmed", "Completed", "Cancelled", "Rescheduled"]
MEETING_TYPES = ["Client Meeting", "Team Meeting", "Advisory Call", "Audit Discussion",
                 "Tax Planning", "Follow-up", "Document Collection", "Other"]
TM_PRIORITY_COLORS = {"Urgent": "#dc3545", "High": "#fd7e14", "Medium": "#e8b86d", "Low": "#28a745"}
TM_STATUS_ICONS    = {"To-Do": "🔲", "In Progress": "🟡", "Paused": "⏸️", "In Review": "🔍", "Done": "✅"}
MEETING_STATUS_COLORS = {
    "Scheduled": "#e8b86d", "Confirmed": "#28a745",
    "Completed": "#5a7a96", "Cancelled": "#dc3545", "Rescheduled": "#fd7e14",
}
...
# ─────────────────────────────────────────────────────────────────────────────
# EXTENDED DB — Calendar & Task Manager tables
# ─────────────────────────────────────────────────────────────────────────────
def init_extended_db():
    ...
    CREATE TABLE IF NOT EXISTS tm_tasks (
        ...
        time_required REAL DEFAULT 30,
        is_running INTEGER DEFAULT 0,
        timer_start REAL DEFAULT 0,
        is_paused INTEGER DEFAULT 0,
        accumulated_mins REAL DEFAULT 0,
        created_at TEXT,
        last_updated TEXT
    )""")
    ...
    CREATE TABLE IF NOT EXISTS tm_scope (
        ...
        category TEXT,
        task_name TEXT,
        actual_mins REAL DEFAULT 0,
        estimated_mins REAL DEFAULT 0,
        completed_at TEXT
    )""")

    # Seed default task categories
    if conn.execute("SELECT COUNT(*) FROM tm_categories").fetchone()[0] == 0:
        for cat in ["ITR Filing", "GST Compliance", "Statutory Audit", "ROC/MCA",
                    "Bookkeeping", "TDS Returns", "Advisory", "Other"]:
            try:
                conn.execute("INSERT INTO tm_categories VALUES (?)", (cat,))
            except Exception:
                pass
    conn.commit()
...
# CALENDAR — helpers
# ─────────────────────────────────────────────────────────────────────────────
def _get_client_list():
    try:
        return pd.read_sql("SELECT client_full_name FROM clients ORDER BY client_full_name",
                           get_connection())["client_full_name"].tolist()
    except Exception:
        return []


def _meetings_for_month(year, month):
    last_day = cal_module.monthrange(year, month)[1]
    return pd.read_sql(
        "SELECT * FROM meetings WHERE meeting_date >= ? AND meeting_date <= ? ORDER BY meeting_date, meeting_time",
        get_connection(),
        params=[f"{year}-{month:02d}-01", f"{year}-{month:02d}-{last_day:02d}"]
    )


def _insert_meeting(data: dict):
    data["id"]         = str(uuid.uuid4())
    data["created_at"] = datetime.now().isoformat()
    data["updated_at"] = datetime.now().isoformat()
    cols = ", ".join(data.keys())
    ph   = ", ".join(["?"] * len(data))
    get_connection().execute(f"INSERT INTO meetings ({cols}) VALUES ({ph})", list(data.values()))
    get_connection().commit()


def _update_meeting(data: dict):
    mid = data.pop("id")
    data["updated_at"] = datetime.now().isoformat()
    set_clause = ", ".join([f"{k}=?" for k in data.keys()])
    conn = get_connection()
    conn.execute(f"UPDATE meetings SET {set_clause} WHERE id=?", list(data.values()) + [mid])
    conn.commit()


def _delete_meeting(mid: str):
    conn = get_connection()
    conn.execute("DELETE FROM meetings WHERE id=?", (mid,))
    conn.commit()


def _render_calendar_html(year, month, df):
    """Render month grid as HTML with colour-coded meeting dots."""
    ...
    if not df.empty:
        for _, row in df.iterrows():
            try:
                d = int(row["meeting_date"].split("-")[2])
    ...
    for week in cal_data:
        for day in week:
            ...
            dots      = ""
            for m in mtgs[:3]:
                c = MEETING_STATUS_COLORS.get(m.get("status", "Scheduled"), "#e8b86d")
                dots += f'<div class="cal-dot" style="background:{c}" title="{m["title"]}"></div>'
            if len(mtgs) > 3:
                dots += f'<div class="cal-more">+{len(mtgs)-3}</div>'
            cells += f'<div class="cal-cell{tc}"><span class="cal-num">{day}</span><div class="cal-dots">{dots}</div></div>'

    hdrs = "".join(f'<div class="cal-hdr">{d}</div>' for d in day_names)
    return f"""
    <style>
    .cal-wrap  {{ display:grid; grid-template-columns:repeat(7,1fr); gap:4px; margin:8px 0 16px; }}
    .cal-hdr   {{ text-align:center; font-size:.75rem; color:#5a7a96; font-weight:700;
                  padding:4px; letter-spacing:.06em; }}
    .cal-cell  {{ background:rgba(255,255,255,.03); border:1px solid rgba(232,184,109,.1);
                  border-radius:6px; padding:6px 8px; min-height:60px; }}
    .cal-today {{ background:rgba(232,184,109,.15); border-color:#e8b86d; }}
    .cal-empty {{ min-height:60px; }}
    .cal-num   {{ font-size:.85rem; font-weight:600; color:#d4e6f1; }}
    .cal-today .cal-num {{ color:#e8b86d; }}
    .cal-dots  {{ display:flex; flex-wrap:wrap; gap:3px; margin-top:4px; }}
    .cal-dot   {{ width:8px; height:8px; border-radius:50%; }}
    .cal-more  {{ font-size:.65rem; color:#7a9bb5; }}
    </style>
    <div class="cal-wrap">{hdrs}{cells}</div>
    """
...
# ─────────────────────────────────────────────────────────────────────────────
def page_calendar():
    st.markdown('<div class="page-title">📅 Client Meeting Calendar</div>', unsafe_allow_html=True)
    init_extended_db()
    ...
    client_list = _get_client_list()

    # ── Month navigation bar ─────────────────────────────────────────────────
    col_p, col_lbl, col_n, _, col_add = st.columns([1, 3, 1, 3, 2])
    if col_p.button("◀", use_container_width=True, key="cal_prev"):
        st.session_state["cal_year"], st.session_state["cal_month"] = \
            (year - 1, 12) if month == 1 else (year, month - 1)
        st.rerun()
    col_lbl.markdown(f"<h3 style='text-align:center;color:#e8b86d;margin:0;"
                     f"font-family:\"Playfair Display\",serif;'>"
                     f"{cal_module.month_name[month]} {year}</h3>", unsafe_allow_html=True)
    if col_n.button("▶", use_container_width=True, key="cal_next"):
        st.session_state["cal_year"], st.session_state["cal_month"] = \
            (year + 1, 1) if month == 12 else (year, month + 1)
        st.rerun()
    if col_add.button("➕ Schedule Meeting", use_container_width=True, type="primary", key="cal_add_btn"):
        st.session_state["show_add_meeting"] = True

    # ── Calendar grid ────────────────────────────────────────────────────────
    month_df = _meetings_for_month(year, month)
    st.markdown(_render_calendar_html(year, month, month_df), unsafe_allow_html=True)

    # ── Add Meeting form ─────────────────────────────────────────────────────
    if st.session_state.get("show_add_meeting"):
        section_hdr("📝 Schedule New Meeting")
        with st.form("add_meeting_form", clear_on_submit=True):
            c1, c2 = st.columns(2)
            m_title  = c1.text_input("Meeting Title *", placeholder="e.g. ITR Review — Q4")
            m_client = c2.selectbox("Client", ["(None)"] + client_list)
            c1, c2, c3 = st.columns(3)
            m_date = c1.date_input("Date *", value=date.today(), format="DD/MM/YYYY")
            m_time = c2.time_input("Time", value=datetime.strptime("10:00", "%H:%M").time())
            m_dur  = c3.number_input("Duration (mins)", min_value=15, max_value=480, value=60, step=15)
            c1, c2 = st.columns(2)
            m_type   = c1.selectbox("Meeting Type", MEETING_TYPES)
            m_status = c2.selectbox("Status", MEETING_STATUS_OPTIONS)
            m_loc    = st.text_input("Location / Video Link")
            m_att    = st.text_input("Attendees (comma-separated)")
            m_desc   = st.text_area("Agenda / Notes", height=80)
            cs, cc   = st.columns(2)
            saved    = cs.form_submit_button("💾 Save Meeting", use_container_width=True, type="primary")
            ...
            if saved:
                if not m_title.strip():
                    ...
                    _insert_meeting({
                        "title": m_title.strip(),
                        "client_name": m_client if m_client != "(None)" else "",
                        "meeting_date": m_date.isoformat(),
                        "meeting_time": m_time.strftime("%H:%M"),
                        "duration_mins": m_dur,
                        "meeting_type": m_type,
                        "location": m_loc,
                        "attendees": m_att,
                    ...
                    st.session_state.pop("show_add_meeting", None)
    ...
    # ── Meeting list for this month ───────────────────────────────────────────
    section_hdr(f"📋 Meetings — {cal_module.month_name[month]} {year}")
    if month_df.empty:
        st.info(f"No meetings scheduled for {cal_module.month_name[month]} {year}.")
    else:
        for _, row in month_df.iterrows():
            sc = MEETING_STATUS_COLORS.get(row.get("status", "Scheduled"), "#e8b86d")
            client_str   = f"🏢 {row['client_name']}" if row.get("client_name") else ""
            location_str = f"&nbsp;📍 {row['location']}"  if row.get("location")    else ""
            attendee_str = f"&nbsp;👥 {row['attendees']}"  if row.get("attendees")   else ""
            desc_str     = (f"<div style='color:#5a7a96;font-size:.78rem;margin-top:3px;'>"
                            f"📝 {row['description']}</div>") if row.get("description") else ""
            st.markdown(f"""
            <div style='border-left:4px solid {sc};background:rgba(255,255,255,.03);
            border-radius:0 8px 8px 0;padding:10px 16px;margin-bottom:6px;'>
                <div style='display:flex;justify-content:space-between;align-items:center;'>
                    <span style='font-weight:700;color:#d4e6f1;'>{row['title']}</span>
                    <span style='background:{sc};color:#1a2a3a;padding:2px 10px;
                    border-radius:12px;font-size:.74rem;font-weight:700;'>{row.get('status','Scheduled')}</span>
                </div>
                <div style='color:#7a9bb5;font-size:.82rem;margin-top:3px;'>
                    📅 {row['meeting_date']} &nbsp;🕐 {row['meeting_time']} &nbsp;⏱ {row['duration_mins']}m
                    &nbsp;{row.get('meeting_type','')}
                    &nbsp;{client_str}{location_str}{attendee_str}
                </div>
                {desc_str}
            </div>
            """, unsafe_allow_html=True)
            ...
            if bc1.button("✏️ Edit",   key=f"medit_{row['id']}", use_container_width=True):
                st.session_state["editing_meeting"]  = row["id"]
                st.session_state.pop("deleting_meeting", None)
            if bc2.button("🗑️ Delete", key=f"mdel_{row['id']}",  use_container_width=True):
                st.session_state["deleting_meeting"] = row["id"]
                st.session_state.pop("editing_meeting", None)

        # ── Edit form ─────────────────────────────────────────────────────────
        eid = st.session_state.get("editing_meeting")
        if eid:
            er_rows = month_df[month_df["id"] == eid]
            if not er_rows.empty:
                er = er_rows.iloc[0]
                section_hdr(f"✏️ Edit: {er['title']}")
                with st.form("edit_meeting_form"):
                    c1, c2 = st.columns(2)
                    e_title  = c1.text_input("Title *", value=er.get("title",""))
                    cn_opts  = ["(None)"] + client_list
                    e_client = c2.selectbox("Client", cn_opts,
                                            index=cn_opts.index(er.get("client_name","")) if er.get("client_name","") in cn_opts else 0)
                    c1, c2, c3 = st.columns(3)
                    e_date = c1.date_input("Date", value=datetime.strptime(er["meeting_date"], "%Y-%m-%d").date(), format="DD/MM/YYYY")
                    t_str  = (er.get("meeting_time") or "10:00")[:5]
                    e_time = c2.time_input("Time", value=datetime.strptime(t_str, "%H:%M").time())
                    e_dur  = c3.number_input("Duration (mins)", min_value=15, value=int(er.get("duration_mins", 60)), step=15)
                    c1, c2 = st.columns(2)
                    mt_idx = MEETING_TYPES.index(er.get("meeting_type","Client Meeting")) if er.get("meeting_type") in MEETING_TYPES else 0
                    e_type   = c1.selectbox("Type", MEETING_TYPES, index=mt_idx)
                    ms_idx   = MEETING_STATUS_OPTIONS.index(er.get("status","Scheduled")) if er.get("status") in MEETING_STATUS_OPTIONS else 0
                    e_status = c2.selectbox("Status", MEETING_STATUS_OPTIONS, index=ms_idx)
                    e_loc    = st.text_input("Location",  value=er.get("location","") or "")
                    e_att    = st.text_input("Attendees", value=er.get("attendees","") or "")
                    e_desc   = st.text_area("Notes",      value=er.get("description","") or "", height=80)
                    cu, cc2  = st.columns(2)
                    if cu.form_submit_button("💾 Update", use_container_width=True, type="primary"):
                        _update_meeting({
                            "id": eid, "title": e_title,
                            "client_name": e_client if e_client != "(None)" else "",
                            "meeting_date": e_date.isoformat(),
                            "meeting_time": e_time.strftime("%H:%M"),
                            "duration_mins": e_dur, "meeting_type": e_type,
                            "location": e_loc, "attendees": e_att,
                            "description": e_desc, "status": e_status,
                        })
                        st.session_state.pop("editing_meeting", None)
        ...
        # ── Delete confirm ────────────────────────────────────────────────────
        did = st.session_state.get("deleting_meeting")
        if did:
            del_rows = month_df[month_df["id"] == did]
            if not del_rows.empty:
                st.warning(f"⚠️ Permanently delete **{del_rows.iloc[0]['title']}**?")
                dc1, dc2, _ = st.columns([1, 1, 6])
                if dc1.button("🔴 Confirm Delete", use_container_width=True, key="conf_del_mtg"):
                    _delete_meeting(did)
                    st.session_state.pop("deleting_meeting", None)
                    st.success("Deleted.")
                    st.rerun()
                if dc2.button("Cancel", use_container_width=True, key="cancel_del_mtg"):
                    st.session_state.pop("deleting_meeting", None)
                    st.rerun()

    # ── Upcoming 7 days ───────────────────────────────────────────────────────
    st.markdown("---")
    section_hdr("🔔 Upcoming — Next 7 Days")
    today_s = date.today().isoformat()
    next7_s = (date.today() + timedelta(days=7)).isoformat()
    upcoming = pd.read_sql(
        "SELECT * FROM meetings WHERE meeting_date >= ? AND meeting_date <= ? "
        "AND status NOT IN ('Completed','Cancelled') ORDER BY meeting_date, meeting_time",
        get_connection(), params=[today_s, next7_s]
    )
    if upcoming.empty:
        st.success("✅ No meetings in the next 7 days.")
    else:
        for _, row in upcoming.iterrows():
            days_away = (datetime.strptime(row["meeting_date"], "%Y-%m-%d").date() - date.today()).days
            label     = "Today" if days_away == 0 else ("Tomorrow" if days_away == 1 else f"In {days_away}d")
            client_p  = f"({row['client_name']})" if row.get("client_name") else ""
            st.markdown(f"- **{row['meeting_date']} {row['meeting_time']}** — {row['title']} "
                        f"{client_p} `{label}` · _{row.get('meeting_type','')}_")


# ─────────────────────────────────────────────────────────────────────────────
# TASK MANAGER — helpers  (logic adapted from Raj Group Vault)
# ─────────────────────────────────────────────────────────────────────────────
def _tm_calc_mins(timer_start, accumulated) -> float:
    acc = float(accumulated or 0)
    if not timer_start or float(timer_start) == 0:
        return round(acc, 2)
    return round(acc + (time.time() - float(timer_start)) / 60, 2)


def _tm_finalize(task_id: str, actual_mins: float):
    conn = get_connection()
    row  = conn.execute(
        "SELECT assigned_to, client, category, task_name, time_required FROM tm_tasks WHERE id=?",
        (task_id,)
    ).fetchone()
    if row:
        assigned_to, client, category, task_name, time_required = row
        conn.execute(
            "INSERT INTO tm_scope (id,assigned_to,date,client,category,task_name,"
            "actual_mins,estimated_mins,completed_at) VALUES (?,?,?,?,?,?,?,?,?)",
            (str(uuid.uuid4()), assigned_to, date.today().isoformat(),
             client, category, task_name, actual_mins,
             float(time_required or 0), datetime.now().isoformat())
        )
        conn.execute(
            "UPDATE tm_tasks SET is_running=0,is_paused=0,timer_start=0,"
            "accumulated_mins=0,status='Done',last_updated=? WHERE id=?",
            (datetime.now().isoformat(), task_id)
        )
        conn.commit()


def _tm_get_categories() -> list:
    return [r[0] for r in get_connection()
            .execute("SELECT category_name FROM tm_categories ORDER BY category_name")
            .fetchall()]
...
def page_task_manager():
    ...
    if _HAS_AUTOREFRESH:
        st_autorefresh(interval=10_000, key="tm_autorefresh")

    auth_user = st.session_state.get("auth_user", "admin")
    auth_role = st.session_state.get("auth_role", "user")
    is_admin  = (auth_role == "admin")
    clients   = _get_client_list()
    cats      = _tm_get_categories()
    conn      = get_connection()

    tabs = st.tabs(["🏃 Active Timers", "📅 My Schedule", "📋 All Tasks", "➕ New Task", "⚙️ Categories"])

    # ── TAB 1 : Active Timers ─────────────────────────────────────────────────
    with tabs[0]:
        section_hdr("🏃 Running & Paused Tasks")
        q  = "SELECT * FROM tm_tasks WHERE (is_running=1 OR is_paused=1)"
        if not is_admin:
            q += f" AND assigned_to='{auth_user}'"
        df_live = pd.read_sql(q, conn)

        if not _HAS_AUTOREFRESH:
            if st.button("🔄 Refresh Timers", use_container_width=True):
                st.rerun()
            st.caption("Tip: `pip install streamlit-autorefresh` for live 10-second updates.")

        if df_live.empty:
            st.info("No tasks currently running or paused.")
        else:
            for _, row in df_live.iterrows():
                cur   = _tm_calc_mins(row["timer_start"], row["accumulated_mins"])
                bgt   = float(row.get("time_required", 0) or 0)
                over  = cur > bgt and bgt > 0
                border_c = "#dc3545" if over else "#28a745"

                st.markdown(f"""
                <div style='border-left:4px solid {border_c};background:rgba(255,255,255,.03);
                border-radius:0 8px 8px 0;padding:10px 16px;margin-bottom:6px;'>
                    <div style='font-weight:700;color:#d4e6f1;'>{row['task_name']}</div>
                    <div style='color:#7a9bb5;font-size:.82rem;'>
                        {row.get('client','')} · {row.get('category','')} · 👤 {row['assigned_to']}
                    </div>
                </div>
                """, unsafe_allow_html=True)

                m1, m2, m3, m4 = st.columns([2, 2, 1, 1])
                m1.metric("Actual", f"{cur}m")
                m2.metric("Budget", f"{bgt}m", delta=f"{round(bgt-cur,1)}m" if bgt > 0 else "—")

                if row["is_running"]:
                    if m3.button("⏸️ Pause", key=f"lv_pause_{row['id']}", use_container_width=True):
                        conn2 = get_connection()
                        conn2.execute("UPDATE tm_tasks SET is_running=0,is_paused=1,timer_start=0,"
                                      "accumulated_mins=?,status='Paused',last_updated=? WHERE id=?",
                                      (cur, datetime.now().isoformat(), row["id"]))
                        conn2.commit(); st.rerun()
                else:
                    if m3.button("▶️ Resume", key=f"lv_resume_{row['id']}", use_container_width=True):
                        conn2 = get_connection()
                        conn2.execute("UPDATE tm_tasks SET is_running=1,is_paused=0,timer_start=?,"
                                      "status='In Progress',last_updated=? WHERE id=?",
                                      (time.time(), datetime.now().isoformat(), row["id"]))
                        conn2.commit(); st.rerun()

                if m4.button("🏁 Finish", key=f"lv_fin_{row['id']}", use_container_width=True, type="primary"):
                    _tm_finalize(row["id"], cur)
                    st.rerun()

    # ── TAB 2 : My Schedule ───────────────────────────────────────────────────
    with tabs[1]:
        section_hdr("📅 Your Upcoming Tasks")
        df_sched = pd.read_sql(
            "SELECT * FROM tm_tasks WHERE assigned_to=? AND status!='Done' ORDER BY due_date, priority",
            conn, params=[auth_user]
        )
        if df_sched.empty:
            st.success("✅ All caught up — no pending tasks!")
        else:
            for _, row in df_sched.iterrows():
                pc = TM_PRIORITY_COLORS.get(row.get("priority", "Medium"), "#e8b86d")
                si = TM_STATUS_ICONS.get(row.get("status", "To-Do"), "🔲")
                with st.expander(f"{si} [{row.get('priority','')}] {row['client']} — {row['task_name']}   |   📅 {row.get('due_date','')}"):
                    sc1, sc2, sc3 = st.columns(3)
                    sc1.markdown(f"**Category:** {row.get('category','')}")
                    sc2.markdown(f"**Budget:** {row.get('time_required',0)}m")
                    sc3.markdown(f"**Status:** {row.get('status','')}")

                    if not row["is_running"] and not row["is_paused"]:
                        if st.button("🚀 Start Timer", key=f"sch_start_{row['id']}", use_container_width=True, type="primary"):
                            conn2 = get_connection()
                            conn2.execute("UPDATE tm_tasks SET is_running=1,timer_start=?,"
                                          "status='In Progress',last_updated=? WHERE id=?",
                                          (time.time(), datetime.now().isoformat(), row["id"]))
                            conn2.commit(); st.rerun()
                    else:
                        st.info("🟢 Timer already running — go to Active Timers tab.")

    # ── TAB 3 : All Tasks ────────────────────────────────────────────────────
    with tabs[2]:
        if is_admin:
            df_all = pd.read_sql("SELECT * FROM tm_tasks ORDER BY priority, due_date", conn)
        else:
            df_all = pd.read_sql("SELECT * FROM tm_tasks WHERE assigned_to=? ORDER BY priority, due_date",
                                 conn, params=[auth_user])

        fc1, fc2, fc3 = st.columns(3)
        f_stat = fc1.multiselect("Status",   TM_STATUS_OPTIONS)
        f_pri  = fc2.multiselect("Priority", TM_PRIORITY_OPTIONS)
        f_cli  = fc3.multiselect("Client",   sorted(df_all["client"].dropna().unique().tolist()) if not df_all.empty else [])

        dff = df_all.copy()
        if f_stat: dff = dff[dff["status"].isin(f_stat)]
        if f_pri:  dff = dff[dff["priority"].isin(f_pri)]
        if f_cli:  dff = dff[dff["client"].isin(f_cli)]

        t_act, t_done = st.tabs(["🎯 Active", "✅ Completed"])

        with t_act:
            df_act = dff[dff["status"] != "Done"]
            if df_act.empty:
                st.success("✅ No active tasks matching filters.")
            for _, tr in df_act.iterrows():
                pc = TM_PRIORITY_COLORS.get(tr.get("priority","Medium"), "#e8b86d")
                si = TM_STATUS_ICONS.get(tr.get("status","To-Do"), "🔲")
                st.markdown(f"""
                <div style='border-left:4px solid {pc};background:rgba(255,255,255,.03);
                border-radius:0 8px 8px 0;padding:8px 14px;margin-bottom:5px;'>
                    <div style='display:flex;justify-content:space-between;'>
                        <span style='font-weight:700;color:#d4e6f1;'>{si} {tr['task_name']}</span>
                        <span style='background:{pc};color:#1a2a3a;padding:1px 8px;
                        border-radius:10px;font-size:.72rem;font-weight:700;'>{tr.get('priority','Medium')}</span>
                    </div>
                    <div style='color:#7a9bb5;font-size:.8rem;margin-top:2px;'>
                        {tr.get('client','')} · {tr.get('category','')} · 👤 {tr['assigned_to']}
                        · ⏱ {tr.get('time_required',0)}m · 📅 {tr.get('due_date','')}
                    </div>
                </div>
                """, unsafe_allow_html=True)

                bc1, bc2, bc3, bc4 = st.columns([1, 1, 1, 6])
                running = bool(tr["is_running"]) or bool(tr["is_paused"])
                if not running:
                    if bc1.button("▶️", key=f"all_start_{tr['id']}", use_container_width=True, help="Start timer"):
                        conn2 = get_connection()
                        conn2.execute("UPDATE tm_tasks SET is_running=1,timer_start=?,"
                                      "status='In Progress',last_updated=? WHERE id=?",
                                      (time.time(), datetime.now().isoformat(), tr["id"]))
                        conn2.commit(); st.rerun()
                else:
                    bc1.markdown(f"<div style='padding-top:6px;font-size:.8rem;color:#e8b86d;'>"
                                 f"{'🟢' if tr['is_running'] else '⏸️'}</div>", unsafe_allow_html=True)

                if bc2.button("✅", key=f"all_done_{tr['id']}", use_container_width=True, help="Mark done"):
                    _tm_finalize(tr["id"], _tm_calc_mins(tr["timer_start"], tr["accumulated_mins"]))
                    st.rerun()

                if bc3.button("🗑️", key=f"all_del_{tr['id']}", use_container_width=True, help="Delete"):
                    st.session_state[f"tm_conf_del_{tr['id']}"] = True

                if st.session_state.get(f"tm_conf_del_{tr['id']}"):
                    st.warning(f"Delete **{tr['task_name']}**?")
                    cf1, cf2 = st.columns(2)
                    if cf1.button("🔴 Confirm", key=f"tm_del_conf_{tr['id']}"):
                        conn2 = get_connection()
                        conn2.execute("DELETE FROM tm_tasks WHERE id=?", (tr["id"],))
                        conn2.commit()
                        st.session_state.pop(f"tm_conf_del_{tr['id']}", None)
                        st.rerun()
                    if cf2.button("Cancel", key=f"tm_del_cancel_{tr['id']}"):
                        st.session_state.pop(f"tm_conf_del_{tr['id']}", None)
                        st.rerun()

        with t_done:
            df_done = dff[dff["status"] == "Done"]
            if df_done.empty:
                st.info("No completed tasks for current filters.")
            for _, tr in df_done.iterrows():
                c0, c1, c2 = st.columns([0.5, 6, 2])
                c0.markdown("✅")
                c1.markdown(f"~~{tr['task_name']}~~ — {tr.get('client','')}")
                if c2.button("↩️ Reopen", key=f"tm_rev_{tr['id']}", use_container_width=True):
                    conn2 = get_connection()
                    conn2.execute("UPDATE tm_tasks SET status='To-Do',last_updated=? WHERE id=?",
                                  (datetime.now().isoformat(), tr["id"]))
                    conn2.commit(); st.rerun()

    # ── TAB 4 : New Task ─────────────────────────────────────────────────────
    with tabs[3]:
        section_hdr("➕ Create New Task")
        with st.form("tm_new_task_form", clear_on_submit=True):
            t_name = st.text_input("Task Description *", placeholder="e.g. Prepare GSTR-3B for October")
            c1, c2 = st.columns(2)
            t_cli = c1.selectbox("Client", ["(General)"] + clients)
            t_cat = c2.selectbox("Category", cats if cats else ["Other"])
            c1, c2 = st.columns(2)
            t_pri = c1.selectbox("Priority", TM_PRIORITY_OPTIONS, index=2)
            t_bgt = c2.number_input("Time Budget (Mins)", min_value=5, value=30, step=5)
            c1, c2 = st.columns(2)
            t_start = c1.date_input("Start Date", value=date.today(), format="DD/MM/YYYY")
            t_due   = c2.date_input("Due Date",   value=date.today(), format="DD/MM/YYYY")
            t_assign = st.text_input("Assign To", value=auth_user,
                                     disabled=not is_admin,
                                     help="Admin can assign to any username")
            if st.form_submit_button("➕ Create Task", use_container_width=True, type="primary"):
                if not t_name.strip():
                    st.error("Task description is required.")
                else:
                    conn2 = get_connection()
                    conn2.execute("""INSERT INTO tm_tasks (id,created_by,assigned_to,client,task_name,
                        ...
                        VALUES (?,?,?,?,?,?,?,?,?,?,?,0,0,0,0,?,?)""",
                        (str(uuid.uuid4()), auth_user,
                         t_assign.strip() or auth_user,
                         t_cli if t_cli != "(General)" else "",
                         t_name.strip(), t_cat,
                         t_start.isoformat(), t_due.isoformat(),
                         "To-Do", t_pri, t_bgt,
                         datetime.now().isoformat(), datetime.now().isoformat()))
                    conn2.commit()
                    st.success(f"✅ Task created and assigned to **{t_assign or auth_user}**.")
                    st.rerun()

    # ── TAB 5 : Categories ───────────────────────────────────────────────────
    with tabs[4]:
        section_hdr("⚙️ Task Categories")
        with st.form("tm_add_cat_form", clear_on_submit=True):
            nc1, nc2 = st.columns([4, 1])
            new_cat = nc1.text_input("New Category")
            if nc2.form_submit_button("Add", use_container_width=True, type="primary"):
                if new_cat.strip():
                    try:
                        conn2 = get_connection()
                        conn2.execute("INSERT INTO tm_categories VALUES (?)", (new_cat.strip(),))
                        conn2.commit()
        ...
        st.markdown("---")
        for cat in _tm_get_categories():
            cc1, cc2 = st.columns([7, 1])
            cc1.markdown(f"📂 {cat}")
            if cc2.button("🗑️", key=f"delcat_{cat}", use_container_width=True):
                conn2 = get_connection()
                conn2.execute("DELETE FROM tm_categories WHERE category_name=?", (cat,))
                conn2.commit(); st.rerun()


# ─────────────────────────────────────────────────────────────────────────────
# SCOPE & EFFICIENCY REPORT PAGE
# ─────────────────────────────────────────────────────────────────────────────
def page_scope_report():
    st.markdown('<div class="page-title">📊 Scope & Efficiency Report</div>', unsafe_allow_html=True)
    init_extended_db()

    df = pd.read_sql("SELECT * FROM tm_scope ORDER BY date DESC", get_connection())
    if df.empty:
        st.info("No completed tasks yet. Finish tasks via Task Manager to see reports here.")
        return

    df["actual_mins"]    = pd.to_numeric(df["actual_mins"],    errors="coerce").fillna(0)
    df["estimated_mins"] = pd.to_numeric(df["estimated_mins"], errors="coerce").fillna(0)
    df["Efficiency %"]   = df.apply(
        lambda x: round(x["estimated_mins"] / x["actual_mins"] * 100, 1) if x["actual_mins"] > 0 else 0,
        axis=1
    )
    df["Variance (m)"] = df["estimated_mins"] - df["actual_mins"]

    # Filters
    c1, c2, c3 = st.columns(3)
    f_user = c1.multiselect("Staff",    df["assigned_to"].dropna().unique().tolist())
    f_cli  = c2.multiselect("Client",   df["client"].dropna().unique().tolist())
    f_cat  = c3.multiselect("Category", df["category"].dropna().unique().tolist())
    d1, d2 = st.columns(2)
    f_from = d1.date_input("From", value=date.today().replace(day=1), format="DD/MM/YYYY")
    f_to   = d2.date_input("To",   value=date.today(),                format="DD/MM/YYYY")

    filt = df.copy()
    if f_user: filt = filt[filt["assigned_to"].isin(f_user)]
    if f_cli:  filt = filt[filt["client"].isin(f_cli)]
    if f_cat:  filt = filt[filt["category"].isin(f_cat)]
    filt = filt[
        (pd.to_datetime(filt["date"], errors="coerce") >= pd.Timestamp(f_from)) &
        (pd.to_datetime(filt["date"], errors="coerce") <= pd.Timestamp(f_to))
    ]

    tot_est = filt["estimated_mins"].sum()
    tot_act = filt["actual_mins"].sum()
    eff     = round(tot_est / tot_act * 100, 1) if tot_act > 0 else 0

    k1, k2, k3, k4 = st.columns(4)
    k1.metric("📋 Tasks Done",      len(filt))
    k2.metric("⏱ Budgeted Hours",   f"{round(tot_est/60, 1)}h")
    k3.metric("⏱ Actual Hours",     f"{round(tot_act/60, 1)}h")
    k4.metric("📈 Efficiency",       f"{eff}%")
    st.markdown("---")

    show_cols = ["date", "assigned_to", "client", "category", "task_name",
                 "estimated_mins", "actual_mins", "Variance (m)", "Efficiency %"]
    valid     = [c for c in show_cols if c in filt.columns]
    st.dataframe(filt[valid].rename(columns={
        "date": "Date", "assigned_to": "Staff", "client": "Client",
        "category": "Category", "task_name": "Task",
        "estimated_mins": "Budget (m)", "actual_mins": "Actual (m)"
    }), use_container_width=True, hide_index=True)

    csv = filt.to_csv(index=False).encode("utf-8")
    st.download_button("📥 Download CSV", data=csv,
                       file_name=f"scope_{date.today().isoformat()}.csv",
...
def main():
    ...
    elif page == "🔍 Search & Filter":     page_search()
    elif page == "📊 Analytics":           page_analytics()
    elif page == "⬇️ Export":              page_export()
    elif page == "📅 Calendar":            page_calendar()
    elif page == "📋 Task Manager":        page_task_manager()
    elif page == "📊 Scope Report":        page_scope_report()
